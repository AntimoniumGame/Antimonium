...
/proc/InitializeAdminDatabase()
	...
	// Init/load DB
	glob.admin_db = new("data/admins.db")
	...
	while(query.NextRow())
		...
		glob.admins[results["ckey"]] = new /datum/admin_rank(results["ckey"], results["permissions"], results["title"])

	// Insert default admins if needed.
	for(var/admin in File2List("data/default_admins.txt"))
		admin = ckey(admin)
		if(!glob.admins[admin])
			AddToAdminDatabase(admin, PERMISSIONS_DEFAULT, "a Default Administrator")
			glob.admins[admin] = new /datum/admin_rank(admin, PERMISSIONS_DEFAULT, "a Default Administrator")
...
/proc/UpdateAdmins()
	for(var/client/player in glob.clients)
		player.SetAdminPermissions(glob.admins[player.ckey])
