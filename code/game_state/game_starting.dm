...
/datum/game_state/starting/Start()
	...
	// No players? No round.
	if(!isnull(glob.config["enforce_minimum_player_count"]) && players_to_allocate_roles.len < glob.config["enforce_minimum_player_count"])
	...
	var/list/remaining_jobs = glob.low_priority_jobs.Copy()

	for(var/thing in glob.high_priority_jobs)
	...
	if(glob.config["enforce_minimum_job_slots"] && glob.high_priority_jobs.len)
	...
	var/list/assigning_antagonists = shuffle(glob.antagonist_datums.Copy())
	if(players_to_allocate_roles.len)
	...
	if(players_to_allocate_roles.len)
		for(var/thing in players_to_allocate_roles)
			var/mob/abstract/new_player/player = thing
			players_to_spawn[player] = glob.default_latejoin_role
