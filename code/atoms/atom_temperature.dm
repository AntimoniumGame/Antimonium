/atom/New()
	...
	if(flags & FLAG_TEMPERATURE_SENSITIVE)
		_glob.temperature_sensitive_atoms += src
...
/atom/proc/EqualizeTemperature()
	...
	if(env_temperature < temperature)
		...
		GainHeat(env_temperature, loc.GetMass() * 10)
...
/atom/proc/GainHeat(var/source_heat = TEMPERATURE_ROOM, var/radiator_size = 1)
	if((flags & FLAG_TEMPERATURE_SENSITIVE) && temperature < source_heat)
		temperature = min(source_heat, temperature + round(((source_heat * radiator_size) / GetMass()) * 0.1))
		return TRUE
	return FALSE

/atom/proc/LoseHeat(var/source_heat = TEMPERATURE_ROOM)
	if((flags & FLAG_TEMPERATURE_SENSITIVE) && temperature > source_heat)
		temperature = max(source_heat, temperature - (min(50,max(1,round(GetMass()/3)))))
		return TRUE
	return FALSE
...
/atom/proc/RadiateHeat(var/amount, var/distance = 1)
	...
	for(var/turf/neighbor in Crange(distance, src))
		neighbor.GainHeat(round(. / max(1,(get_dist(src, neighbor)*2)-1)), GetMass())
