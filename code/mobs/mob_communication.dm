...
/mob/verb/Say(var/message as text)
	...
	if(copytext(message,1,2) == "/")
		...
		words.Cut(1,2)
		if(prefix && _glob.all_chat_commands[prefix])
			var/datum/chat_command/command = _glob.all_chat_commands[prefix]
...
/mob/proc/NotifyDead(var/message)
	...
	for(var/thing in _glob.dead_mob_list)
		var/mob/deadite = thing
		if(deadite.client)
			notified += deadite.client
			deadite.Notify("DEAD: [message]")
	for(var/thing in (_glob.clients-notified))
