	...
	for(var/slot in inventory_slots_by_layer)
		if(inventory_slots_by_layer[slot])
			...
			if(istype(inv_slot) && inv_slot.holding)
				...
				if(prone)
					supplied += inv_slot.holding.get_prone_worn_icon(effective_ident)
				else
					supplied += inv_slot.holding.get_worn_icon(effective_ident)
	..(supplied)
